...
def select_style_img_nb(frame_nb, frequency=5, frames_per_second=24):
    ...
    type: frequency: int;
    param: frequency: Number of frames to be translated using a single style image;
    type: frames_per_second: int;
    param: frames_per_second: number of frames per second in the source video;'''
...
if __name__ == "__main__":
    ...
    parser.add_argument('--style_frequency', type=int, default=5,
                        help='Frequency (in s) at which change of style image')
    ...
    stylized_path = os.path.join(
        opt.save_folder,  opt.video_folder.split('/')[-1] + '_stylized')
    ...
    for i in tqdm(sorted(os.listdir(opt.video_folder))):
        ...
        if os.path.isfile(filepath):
            ...
            content = open_image(os.path.join(
                opt.video_folder, i), opt.image_size).to(device)
            style_img_nb = select_style_img_nb(
                frame_nb, opt.style_frequency, opt.frames_per_second)
            style = open_image(os.path.join(generated_path, str(
                style_img_nb) + '_G.png'), opt.image_size).to(device)
            ...
            with torch.no_grad():
                img = wct2.transfer(
                    content, style, content_segment, style_segment, 1)
            save_image(img.clamp_(0, 1), os.path.join(
                stylized_path, i), padding=0)
