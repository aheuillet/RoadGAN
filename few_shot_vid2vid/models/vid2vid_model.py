...
class Vid2VidModel(BaseModel):
    ...
    def langevin_dynamics_sampler(self, N, z0, prevs, tgt_labels, ref_labels, ref_images, tgt_image, eps=0.01):
        ...
        for i in range(N):
            ...
            lipschitz_value = [loss.cpu().clone().numpy() for loss in lipschitz_value]
            print("Lipshitz: ", -np.log(z0)-logit(np.array(lipschitz_value)), flush=True) #Convert each torch loss tensor into a numpy array
    ...
    def inference(self, tgt_label, ref_labels, ref_images):
        ...
        print("TGT LABEL", tgt_label_valid)
        with torch.no_grad():
